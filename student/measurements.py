...
class Sensor:
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            if x[0] == 0:
                raise ValueError('The x value is 0, cannot convert to vehicle coordinates.')
            pos_veh = np.ones((4, 1)) # homogeneous coordinates
            pos_veh[0:3] = x[0:3] 
            pos_sens = self.veh_to_sens*pos_veh # transform from vehicle to lidar coordinates
            proj_sens = np.zeros((2,1))
            proj_sens[0]= self.c_i - (self.f_i*pos_sens[1])/pos_sens[0]
            proj_sens[1]= self.c_j - (self.f_j*pos_sens[2])/pos_sens[0]
            ############
