...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    if len(detections)>all_positives:
...
def compute_performance_stats(det_performance_all):
    ...
    positives = np.sum(pos_negs,axis = 0)[0]
    true_positives = np.sum(pos_negs,axis = 0)[1]
    false_negatives = np.sum(pos_negs,axis = 0)[2]
    false_positives = np.sum(pos_negs,axis = 0)[3]
